#' This function allows you to optimize comfert
#' @param res_dir directory where the process leave the resualts.
#' @param country .
#' @param endY .
#' @param iniY .
#' @param ini_c .
...
optimize_comfert <- function(res_dir, country, iniY, endY, ini_c, n0, nsim,
  ...
  obs <- comfert:::get_obs(country, ysd) # observed data
  ...
  while (n < (n0+N)) {
    ...
    sim <- comfert::get_sim(res_dirs, iniY, endY, nsim, obs,
    ...
    new_set <- comfert::get_new_points(priors, country, n = n0)
    ...
    output <- comfert::parallel_comfert(params = new_eval[rep(1:nrow(new_eval), each = nsim),],
                               country,
                               ini_c,
                               iniY,
                               endY)
    ...
    # simulated rates
    sim <- comfert::get_sim(res_dirs_ne, iniY, endY, nsim, obs,
                   unplanned = length(obs$obs_unplanned)>0,
                   unwanted = length(obs$obs_unwanted)>0,
                   desired = length(obs$obs_desired)>0,

...
# FIXED PARAMETERS                                        #
###########################################################
lambda <- 2.5e-08  # rate decrease penalty intention after birth
epsilon <- 0.53    # rate effect education
sd_lnrm <- 0.16    # stdrd dev lognorm
gamma <- 38.5      # Fecundability age
kappa  <- 0.6      # Fecundability rate
A <- 0.07          # reduction risk unplanned after achieve D
phi <- .22         # maximum fecundability
theta <- 0.7       # scale of truncated Gamma (D)
u <- c(seq(0.25, 0.1, length.out = 1940 - iniY),
           seq(0.1, 0.28, length.out = (endY + 1) - 1940)) # union probability
###########################################################
...
aux_mau <- aux_myedu <- aux_myedu_2 <- dKids <- dKids_all <-  ppBirths <- meanIntentB  <- meanNrDate <- vector()
gapKids <- gapKids3 <- gapEdu1 <- gapEdu2 <- gapEdu3 <- gapBread1 <- gapDual3 <- totPop <-vector()
gap0Prob <- gap0Prob_e1 <- gap0Prob_e2 <- gap0Prob_e3 <- gap_pos <- gap_pos_e1 <- vector()

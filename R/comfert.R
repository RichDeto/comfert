#' \code{confert} package
#'
#' Caja de Herramientas para el computo de fertilidad mediante AMB y ABC
#'
#' See the README on
#' \href{https://github.com/RichDeto/comfert/blob/master/README.md}{Github}
#'
#' @docType package
#' @name ech
NULL
...
#' This function allows you to process the resaults
#' @param seed_val value of the seed.
#' @param param .
#' @param parallel_run . Default F
#' @param cluster . Default F
#' @keywords process
#' @return list
#' @export
#' @family  analysis
#' @examples
#' .
comfert <- function(seed_val, param, parallel_run = F, cluster = F){
  ...
  while (time < endTime){
    ...
    if (year != year(time)){
      ...
      trend_d <- c(trend_d, mean(pop[age %in% minAge:maxAge, desiredKids], na.rm = T))
  ...
  if(cluster){
    out <- list(iniYear = iniYear, endYear = endYear, asfrt = asfrt,
                ...
                cohort = cohort, cohort1 = cohort1, cohort2 = cohort2, cohort3 = cohort3)
  }else{

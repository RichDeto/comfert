...
country <- "ES" # country on which simulations are performed ("FR" or "ES")
iniY <- 1910 # years for which the simulations are performed
endY <- 2017 # years for which the simulations are performed
ysd <- 1960  # year begin computing indicators
ini_c <- 200 # size of the initial birth cohorts of the model (affects computation times - 50 takes 10-15 minutes, but should be closer to 1000 for smooth results)
n0 <- 50 # size of initial sample of param combinations
...
N <- 50 # total nr of evaluations n0+N
...
params <- get_new_points(priors, country, n=n0) # Initial sample of parameter combinations  
